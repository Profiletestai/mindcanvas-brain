...
function normalizeBlock(block: any): ReportBlock {
  return {
    title: safeString(block?.title),
    short_summary: safeString(block?.short_summary),
    paragraphs: Array.isArray(block?.paragraphs)
      ? block.paragraphs.map((p: any) => safeString(p)).filter(Boolean)
      : [],
    bullets: Array.isArray(block?.bullets)
      ? block.bullets.map((b: any) => safeString(b)).filter(Boolean)
      : [],
    items: Array.isArray(block?.items)
      ? block.items.filter((x: any) => x && typeof x === "object")
      : [],
    transition: safeString(block?.transition),
...
function normalizeSections(rawSections: any[]): ProfileExtendedSection[] {
  const mapped: Array<ProfileExtendedSection | null> = (rawSections || []).map(
    (section: any) => {
      ...
      const panels: ProfileExtendedPanel[] = Array.isArray(section?.panels)
        ? section.panels
            .map((panel: any): ProfileExtendedPanel | null => {
              ...
              const blocks = Array.isArray(panel?.blocks)
                ? panel.blocks.map((b: any) => normalizeBlock(b))
                : [];
...
function ItemCards({ items }: { items: Array<Record<string, any>> }) {
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {items.map((item, idx) => {
        const entries = Object.entries(item).filter(
          ([, value]) => safeString(String(value || "")) !== ""
        );
...
function BlockRenderer({
  ...
  const showBlockTitle = !!blockTitle && blockTitle !== panelTitle;
  ...
  return (
    <div className="space-y-4">
      ...
      {items.length > 0 ? <ItemCards items={items} /> : null}
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 space-y-6">
        {blocks.map((block: ReportBlock, idx: number) => (
          <BlockRenderer key={idx} block={block} panelTitle={title} />
        ))}
      </div>
    </div>
  );
...
function SectionRenderer({ section }: { section: ProfileExtendedSection }) {
  return (
    <WhiteCard id={safeSectionId(section.section_key)} title={section.heading}>
      <div className="space-y-5">
        {section.panels.map((panel: ProfileExtendedPanel) => (
      ...
      <div className="bg-slate-100">
        <div ref={rootRef} className="mx-auto max-w-[1440px] px-5 pb-16 pt-5">
          ...
          <div className="mt-4 grid gap-4 xl:grid-cols-[280px_minmax(0,1fr)] items-start">
            ...
            <div className="space-y-4">
              {sections.map((section: ProfileExtendedSection) => (
