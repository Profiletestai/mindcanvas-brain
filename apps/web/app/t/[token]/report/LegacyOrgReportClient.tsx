...
type LegacyResultData = {
  ...
  frequency_labels: FrequencyLabel[];
  frequency_percentages: Partial<Record<FrequencyCode, number>>;
  profile_labels: ProfileLabel[];
  profile_percentages: Record<string, number>;
  top_freq: FrequencyCode;
  top_profile_code: string;
...
type LegacyResultAPI = { ok: boolean; data?: LegacyResultData; error?: string };

function normalisePercentMap<T extends string>(input: Partial<Record<T, number>> | null | undefined) {
  const source = (input || {}) as Partial<Record<T, number>>;
  const values = Object.values(source).filter(
    (value): value is number => typeof value === "number" && Number.isFinite(value),
  );
  const shouldMultiply = values.length > 0 && Math.max(...values) <= 1;
  const output: Partial<Record<T, number>> = {};

  Object.entries(source).forEach(([key, raw]) => {
    const value = typeof raw === "number" && Number.isFinite(raw) ? raw : 0;
    output[key as T] = Math.round(shouldMultiply ? value * 100 : value);
  });
...
function normaliseProfileCode(code?: string | null) {
  const raw = String(code || "").trim().toUpperCase();
  const match = raw.match(/^(?:PROFILE_)?P?(\d+)$/);
  if (match) return `PROFILE_${match[1]}`;
  return raw;
}

function adaptLegacyResultData(input: LegacyResultData): ReportData {
  const frequencyPercentages = normalisePercentMap<FrequencyCode>(
    input.frequency_percentages,
  ) as Record<FrequencyCode, number>;
  const profilePercentages = normalisePercentMap<string>(
    input.profile_percentages,
  ) as Record<string, number>;

  Object.entries({ ...profilePercentages }).forEach(([key, value]) => {
    const normalisedKey = normaliseProfileCode(key);
    const shortKey = profileCodeToShort(normalisedKey);
    profilePercentages[normalisedKey] = Math.round(Number(value || 0));
    profilePercentages[shortKey] = Math.round(Number(value || 0));
  });

  const profileLabels = (input.profile_labels || []).map((profile) => {
    const code = normaliseProfileCode(profile.code);
    return {
      ...profile,
      code,
      frequency_code:
        profile.frequency_code ||
        PROFILE_FREQUENCIES[code] ||
        null,
    };
  });

  const sortedProfiles = [...profileLabels]
    .map((profile) => ({
      ...profile,
      pct: Math.round(
        profilePercentages[profile.code] ||
          profilePercentages[profileCodeToShort(profile.code)] ||
          profilePercentages[normaliseProfileCode(profile.code)] ||
          0,
      ),
  ...
  const topFreqName =
    input.frequency_labels?.find((item) => item.code === input.top_freq)?.name ||
    FREQUENCY_CONTENT[input.top_freq]?.name ||
    input.top_freq;

  return {
    hidden: input.link?.show_results === false,
    org: {
      id: null,
      slug: input.org_slug || null,
      name: input.org_name || "Life Puzzle",
      website_url: null,
    },
    test: {
      id: "legacy-native-report",
      name: input.test_name || "Team Puzzle Discovery Assessment",
      slug: null,
      meta: {},
    },
    ...
    taker: {
      id: input.taker.id,
      first_name: input.taker.first_name || input.taker.firstName || null,
      last_name: input.taker.last_name || input.taker.lastName || null,
      email: input.taker.email || null,
      company: input.taker.company || null,
      role_title: input.taker.role_title || null,
    },
    submission: null,
    result: {
      frequency_labels: input.frequency_labels || [],
      frequency_percentages: frequencyPercentages,
      profile_labels: profileLabels,
      profile_percentages: profilePercentages,
      top_freq: input.top_freq,
      top_freq_name: topFreqName,
      top_profile_code: normaliseProfileCode(input.top_profile_code),
      top_profile_name: input.top_profile_name,
      sorted_profiles: sortedProfiles,
      secondary_profile: sortedProfiles[1] || null,
      tertiary_profile: sortedProfiles[2] || null,
    },
    rhythm: null,
  };
}
...
export default function LegacyOrgReportClient(props: {
  token: string;
  tid: string;
  src?: string;
}) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (isPortalViewer) qs.set("src", "portal");

        const res = await fetch(
          `/api/public/test/${encodeURIComponent(token)}/report?${qs.toString()}`,
          {
            cache: "no-store",
          },
        );
        ...
        const json = (await res.json()) as LegacyResultAPI;
        if (!res.ok || !json.ok || !json.data)
          throw new Error(json.error || `HTTP ${res.status}`);
        if (cancelled) return;

        const adapted = adaptLegacyResultData(json.data);

        if (adapted.hidden && !isPortalViewer) {
          const redirect = adapted.link?.redirect_url?.trim();
          if (redirect) {
  ...
  async function handleDownloadPdf() {
    ...
    try {
      ...
      pdf.save(`team-puzzle-discovery-report-${token}.pdf`);
    } catch (e) {
            ...
            <Card>
              ...
              <WhiteCard>
                ...
                <div className="mt-6 text-sm text-slate-500">
                  Team Puzzle Discovery Assessment
                </div>
              </WhiteCard>
            </Card>
          </div>
