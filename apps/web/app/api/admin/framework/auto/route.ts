...
export async function POST(req: Request) {
  try {
    ...
    const name =
      `${body.orgName || "Signature"} — Core Framework`;
    ...
    const { data: existing } = await supabase
      .from("frameworks")
      .select(
        "id, org_id, name, version, created_at, owner_id, frequency_meta"
      )
      .eq("org_id", orgId)
      .limit(1);
    ...
    return NextResponse.json(
      {
        error: e?.message ?? "Auto framework failed",
      },
      {
        status: 400,
      }
