...
export async function GET(req: Request) {
  try {
    ...
    // 1) Find submissions for this link within range (includes company)
    ...
    // submission -> company from expanded
    const companyBySub = new Map<string, string | null>();
    for (const r of expanded) {
      if (!r?.submission_id) continue;
      companyBySub.set(r.submission_id, r.company ?? null);
    }
