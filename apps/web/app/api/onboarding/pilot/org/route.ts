// Pilot org creation — mirrors v2 onboarding org, but stamps account_type='pilot'
// so the activation step (and later UI) can distinguish pilot orgs.
import { NextResponse } from "next/server";
import { portalAdmin } from "@/app/_lib/supabaseAdmin";
import { getAuthUser } from "../../v2/_lib/auth";
import { generateUniqueSlug } from "../../v2/_lib/slug";
...
export async function POST(req: Request) {
  try {
    ...
    if (existing?.org_id) {
      // Ensure the marker is present even on resume.
      await admin.from("orgs").update({ account_type: "pilot" }).eq("id", existing.org_id);
    ...
    const { error: consentError } = await admin
      ...
      .eq("id", orgId);
    if (consentError) {
      console.error("[onboarding/pilot/org] consent/account_type update failed", consentError);
    }
