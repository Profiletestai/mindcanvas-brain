...
function pickSummary(sections: any[]) {
  ...
  const firstText = (sec: any) => {
    ...
    if (
      typeof b.short_summary === "string" &&
      b.short_summary.trim()
    ) {
...
function hasPillarSignals(result: any) {
  ...
  const okScores =
    ...
    (Object.keys(ps).length > 0 ||
      ["discoverability", "trust", "conversion"].some(
        (k) => ps?.[k] != null
      ));
...
export async function GET(
  ...
  try {
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "Missing org slug. Pass ?org=whatswhats-global",
        },
        { status: 400 }
      );
    }

    // Authenticate the caller and verify that they may read
    // the requested organisation before using privileged clients.
    ...
    const authorisedOrgId = access.access.org.id;

    const sb = portal();
    const vis = visibility();

    // 1) Load only the organisation already authorised above.
    const { data: org, error: orgErr } = await sb
      .from("orgs")
      .select("id, slug, name, logo_url")
      .eq("id", authorisedOrgId)
      .maybeSingle();
    ...
    // 2) Load the taker only if they belong to the
    // authorised organisation.
    const { data: taker, error: takerErr } = await sb
      .from("test_takers")
      .select(
        "id, org_id, test_id, first_name, last_name, email, link_token"
      )
      .eq("id", takerId)
      .eq("org_id", authorisedOrgId)
      .maybeSingle();

    if (takerErr) {
      throw new Error(takerErr.message);
    }
    ...
    // 3) Find the latest visibility submission for this taker.
    // Limit the privileged query to the authorised organisation
    // before inspecting submission metadata.
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "No visibility submission found for this taker yet.",
        },
        { status: 404 }
      );
    }
    ...
    // submissionId came from an org-scoped submissions query above.
    ...
    if (!result) {
      return NextResponse.json(
        {
          ok: false,
          error: "Visibility results not found.",
        },
        { status: 404 }
      );
    }

    const engineKey = String(
      result.engine_key || "visibility_v1"
    );
    ...
    // 5) Cache lookup — but only return it if it has pillars
    // (otherwise regenerate).
    const cached = await callRpc<any>(
      vis,
    ...
    // 6) Ensure pillar signals exist on results
    // (compute via RPC if missing).
      ...
      try {
        ...
        if (
          ...
          await vis
            ...
            .update({
              pillar_scores:
                computed.pillar_scores ??
                {},
              pillar_bands:
                computed.pillar_bands ??
                {},
              weakest_pillar:
                computed.weakest_pillar ??
                null,
              strongest_pillar:
                computed.strongest_pillar ??
                null,
              pattern_tags:
                Array.isArray(
        ...
        console.warn(
          "[visibility snapshot] pillar RPC failed",
          e
        );
      }
    }
